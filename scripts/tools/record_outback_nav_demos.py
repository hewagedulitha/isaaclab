...
def main():
    ...
    with contextlib.suppress(KeyboardInterrupt) and torch.inference_mode():
        while True:
            ...
            prev_obs = obs
            obs, rew, done, time_out, _ = env.step(actions)
            state_dicts.append({
                "s" : prev_obs["policy"].detach().cpu().numpy().tolist(),
                "a": actions.detach().cpu().numpy().tolist(),
                "next_s": obs["policy"].detach().cpu().numpy().tolist(),
                "r": rew.detach().cpu().numpy().tolist(),
                "done": done.detach().cpu().numpy().tolist(),
                "time_out": time_out.detach().cpu().numpy().tolist(),
            })

            if done:
                print("[INFO] saving state buffer")
                with open(f'datasets/result_{demo_count}.json', 'w') as fp:
